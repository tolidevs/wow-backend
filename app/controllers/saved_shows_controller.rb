class SavedShowsController < ApplicationController
    ...
    def create
        if !SavedShow.all.find{|show| show.user_id == params[:user] && show.imdbID == params[:imdbID]}
            @saved_show = SavedShow.create(user_id: params[:user], imdbID: params[:imdbID], title: params[:title], show_type: params[:show_type], year: params[:year], poster: params[:poster])
